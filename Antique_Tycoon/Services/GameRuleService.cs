...
public class GameRuleService : ObservableObject
{
  ...
  private async Task<bool> HandleStepOnNodeAsync(Player player, NodeModel node)
  {
    switch (node)
    {
      ...
      case Ender:
        await HandelEnderAsync(player, node);
        return false;
  ...
  private async Task HandelEnderAsync(Player player, NodeModel node)
  {
    if (player.Staffs.Count < 1)
      return;
  ...
  private async Task HandelEnderChestAsync(Player player, NodeModel node) //todo 应该投骰子来决定是否成功
  {
    ...
    var players = _gameManager.Players.Where(p => p != player && p.Antiques.Count > 0).Select(p => p.Uuid).ToList();
    if (players.Count == 0)
      return;
      ...
      if (string.IsNullOrEmpty(plunderRequest.AntiqueUuid))
      {
        if (client != null)
          await _gameManager.NetServerInstance.SendResponseAsync(
            new AcknowledgementResponse(plunderRequest.Id), client);
      }
      else
      {
        ...
        var rollDiceResponse = new RollDiceResponse(plunderRequest.Id, player.Uuid, diceContext.Result)
        {
          ...
          [
            ...
            new LogSegment { Text = actionText },
            new LogSegment { Type = InteractionType.PlayerName, Data = owner.Uuid },
            new LogSegment { Text = " 的 " },
            new LogSegment { Type = InteractionType.Antique, Data = antique.Uuid }
          ]
        };
        await Broadcast(rollDiceResponse);
