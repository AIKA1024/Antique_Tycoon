...
public partial class PlayerUiViewModel : PageViewModelBase, IDisposable
{
  ...
  public partial ObservableCollection<Player> OtherPlayers { get; set; } =
    [new Player { Name = "lbw" }, new Player { Name = "lbw" }, new Player { Name = "lbw" }];
  ...
  private void ReceiveInitGameMessage(object recipient, InitGameResponse message)
  {
    OtherPlayers = new ObservableCollection<Player>(_gameManager.Players.Where(p => p != _gameManager.LocalPlayer));
  }
  ...
  private void ReceiveUpdateRoomResponse(object recipient, UpdateRoomResponse message)
  {
    foreach (var data in message.Players)
    {
      ...
      var player = _gameManager.Players.First(p => p.Uuid == data.Uuid);
  ...
  [RelayCommand]
  private async Task ShowPlayerPropertiesDialog(Player player)
  {
    await _dialogService.ShowDialogAsync(new PlayerPropertiesDialogViewModel(player));
  }

  [RelayCommand]
  private void CloseDialogByMaskTap()
  {
    if (DialogViewModel is { IsLightDismissEnabled : false })
      return;

    if (IsDerivedFromGenericDialogViewModelBase(DialogViewModel.GetType()))
      _dialogService.CloseDialogsAndClearResults(DialogViewModel);
    else
      _dialogService.CloseDialog(DialogViewModel);
  }
